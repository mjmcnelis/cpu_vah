...
#define TEST_TMUNU 1		// 1 for test
	...
	for(int k = 2; k < nz + 2; k++)
	{
		for(int j = 2; j < ny + 2; j++)
		{
			for(int i = 2; i < nx + 2; i++)
			{
				...
				// quadratic formula
				precision e_s = fmax(E_MIN, (sqrt(fabs(b * b  -  4.0 * a * c))  -  b) / (2.0 * a));
				//
			...
			#if (TEST_TMUNU == 1)
				precision utperp_s = sqrt(1.0  +  ux_s * ux_s  +  uy_s * uy_s);

				precision zt_s = t * un_s / utperp_s;
				precision zn_s = ut_s / (t * utperp_s);

				precision dttt = fabs((e_s + pt) * ut_s * ut_s  -  pt  +  (pl - pt) * zt_s * zt_s  +  2.0 * WtTz * zt_s  +  pitt  -  ttt);
