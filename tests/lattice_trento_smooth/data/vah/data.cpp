...
int main()
{
    ...
    double times[17] = {0.05, 0.55, 1.05, 2.05, 3.05, 4.05, 5.05, 6.05, 7.05, 8.05, 9.05, 10.05, 11.05, 12.05, 13.05, 14.05, 15.05};			// hard coded
    ...
    for(int n = 0; n < 17; n++)
    {
        ...
        sprintf(fname7, "../../../../output/shear_peq_%.3f.dat", t);
        sprintf(fname8, "../../../../output/bulk_peq_%.3f.dat", t);
        ...
        shear_3d  = fopen(fname7, "r");
        bulk_3d   = fopen(fname8, "r");
        ...
        sprintf(fname7,  "x_axis/shear_peq_%.3f.dat", t);
        sprintf(fname8, "x_axis/bulk_peq_%.3f.dat", t);
		...
		ux_x     = fopen(fname5, "w");
		un_x     = fopen(fname6, "w");
        shear_x  = fopen(fname7, "w");
        bulk_x   = fopen(fname8, "w");
        ...
        sprintf(fname7, "z_axis/shear_peq_%.3f.dat", t);
        sprintf(fname8, "z_axis/bulk_peq_%.3f.dat", t);
		...
		ux_z     = fopen(fname5, "w");
		un_z     = fopen(fname6, "w");
        shear_z  = fopen(fname7, "w");
        bulk_z   = fopen(fname8, "w");
    	...
    	for(int k = 0; k < nz; k++)
    	{
    		for(int j = 0; j < ny; j++)
    		{
    			for(int i = 0; i < nx; i++)
    			{
    				...
    				double energy, plpt, piperp, Wperp, ux, un, shear, bulk;
                    ...
                    fscanf(shear_3d,  "%lf\t%lf\t%lf\t%lf", &x, &y, &z, &shear);
                    fscanf(bulk_3d,   "%lf\t%lf\t%lf\t%lf", &x, &y, &z, &bulk);
    				...
    				if(j == (ny - 1)/2 && k == (nz - 1)/2)
    				{
    					...
    					fprintf(ux_x,     "%.2f\t%.4e\n", x, ux);
    					fprintf(un_x,     "%.2f\t%.4e\n", x, un);
                        fprintf(shear_x,  "%.2f\t%.4e\n", x, shear);
                        fprintf(bulk_x,   "%.2f\t%.4e\n", x, bulk);
       				}
       				...
       				if(i == (nx - 1)/2 && j == (ny - 1)/2)
    				{
    					...
    					fprintf(ux_z,     "%.2f\t%.4e\n", z, ux);
    					fprintf(un_z,     "%.2f\t%.4e\n", z, un);
                        fprintf(shear_z,  "%.2f\t%.4e\n", z, shear);
                        fprintf(bulk_z,   "%.2f\t%.4e\n", z, bulk);
       				}
